...
const margin = {top: 20, right: 30, bottom: 30, left: 50},
   ...
   const svg = d3.select("#anomalieschart")
  .append("svg")
  .attr("width", width + margin.left + margin.right)
  .attr("height", height + margin.top + margin.bottom)
  .attr("style", 
    "max-width: 700px; height: auto; overflow: visible; font: 0.8rem sans-serif; color: black; background-color: #f7f5ed; align-items: center; margin-bottom: 0px;"
  );
    ...
    d3.csv("data/wales_annual_rain_temp_anomalies.csv").then(data => {
      ...
      const x = d3.scaleLinear()
        .domain(d3.extent(data, d => d.rain_anom_mm)).nice()
        .range([0, width]);

      const y = d3.scaleLinear()
        .domain(d3.extent(data, d => d.temp_anom_C)).nice()
        .range([height, 0]);
      ...
      g.append("text")
        ...
        .attr("font-size", "14px")
        .text("Annual Rainfall (mm)"); // Annual Rainfall Anomaly (mm, vs 1991–2020)

      g.append("text")
        .attr("x", 5).attr("y", -20)
        .attr("text-anchor", "middle")
        .attr("font-size", "14px")
        .text("Annual Temperature (°C)"); // Annual Temperature Anomaly 
